...
const getLectureIndex = (x) => {
  ...
  for (let i = 0; i < ranges.length; ++i)
    if (x >= ranges[i] && x <= ranges[i] + 115) return i;
  return -1;
};
...
const getLectureStartIntTime = (x) => {
  ...
  for (let i = 0; i < ranges.length; ++i)
    if (x >= ranges[i] && x <= ranges[i] + 115) return ranges[i];
  return 0;
};
...
const getProgressRatio = (time) => {
  ...
  var passedTime = 0;
  if (time < sTime + 100) passedTime = time - sTime;
  else passedTime = 60 + time - sTime - 100;
            ...
            {lectureId ? (
              <>
                ...
                <IconTextTime
                  text={
                    "수업 시간 : " +
                    getLectureStartStrTime(getCurIntegerTime(date)) +
                    " ~ " +
                    getLectureEndTime(getCurIntegerTime(date))
                  }
                />
                <LectureProgressBar
              ...
              <div style={{ paddingTop: "200px" }}>
                <LectureTitle text="현재 수업중인 강의가 없습니다." />
              </div>
            )}
          </div>
        </div>
